 ...
 */
export function checkGlobalRedFlags(
  intake: IntakeData,
  encounter: Encounter,
  _patient: Patient,
): GlobalRedFlag[] {
  const r = intake.complaint_responses;
  const triggered: GlobalRedFlag[] = [];
  ...
  const severeBreathing =
    r["breathing_severity"] === "Severe (single words only)" ||
  ...
  if (
    encounter.complaint_code === ComplaintCode.MENTAL_HEALTH &&
    (r["plan_to_harm"] === "Yes" || r["plan_to_harm"] === true)
  ) {
    triggered.push({
      name: "active_suicidal_plan",
      instructions:
        "If you are in immediate danger, call 911. Talk Suicide Canada: 988 (call or text). Go to your nearest emergency department.",
    });
  }
  ...
  if (r["chemical_exposure"] === "Yes" || r["chemical_exposure"] === true) {
    triggered.push({
      ...
      instructions:
        "Flush your eye with clean water continuously until you reach the ED.",
    });
  }

  // 6. Airway compromise (cannot swallow + drooling)
  if (
    r["can_swallow"] === "Cannot swallow at all" &&
  ...
  if (
    r["active_bleeding"] === "Severe (won't stop)" ||
  ...
  const tongueOrThroat =
    includes(swelling, "Tongue") || includes(swelling, "Throat");
  if (
    tongueOrThroat &&
  ...
  // 9. Suspected stroke
  if (
    (r["weakness_one_side"] === "Yes" || r["weakness_one_side"] === true) &&
    (r["speech_difficulty"] === "Yes" || r["speech_difficulty"] === true)
  ) {
    triggered.push({
      name: "suspected_stroke",
      instructions:
        "Call 911. Note the time symptoms started. Do not drive.",
    });
  }

  // 10. Neonatal fever (< 3 months + temp >= 38.0)
  const childAgeMonths = computeChildAgeMonths(r);
  const temp = typeof r["temperature"] === "number" ? (r["temperature"] as number) : null;
  if (
    ...
    triggered.push({
      name: "neonatal_fever",
      instructions:
        "Take your baby to the nearest ED immediately.",
    });
  }

  // 11. Meningococcal rash
  const hasFever =
    r["fever"] === "Yes" || r["fever"] === true || (temp !== null && temp >= 38.0);
  if (
    encounter.is_pediatric_flag &&
    ...
    triggered.push({
      ...
      instructions:
        "This could be serious. Go to the nearest ED immediately.",
    });
  }

  // 12. Late pregnancy bleeding
  const weeks = typeof r["weeks_pregnant"] === "number" ? (r["weeks_pregnant"] as number) : 0;
  if (
    r["pregnant"] === "Yes" &&
    weeks > 20 &&
  ...
  if (
    ...
    triggered.push({
      ...
      instructions:
        "Go to the nearest ED immediately. This is time-sensitive.",
    });
  }

  // 14. Cauda equina (back pain + bladder/bowel)
  if (
    encounter.complaint_code === ComplaintCode.BACK_JOINT &&
    (r["bladder_bowel"] === "Yes" || r["bladder_bowel"] === true)
  ) {
    triggered.push({
      name: "cauda_equina",
      instructions:
        "Go to the nearest ED immediately. This may require urgent surgery.",
    });
  }
...
function computeChildAgeMonths(r: Record<string, unknown>): number | null {
  if (typeof r["child_age_months"] === "number") {
    return r["child_age_months"] as number;
  }
  if (typeof r["child_age_years"] === "number") {
    return (r["child_age_years"] as number) * 12;
  }
  return null;
}
