...
const evaluators: Record<ComplaintCode, Evaluator> = {
  ...
  [ComplaintCode.FEVER]: (intake) => {
    ...
    if (typeof r["temperature"] === "number" && (r["temperature"] as number) >= 40) {
      mods.push({
  ...
  [ComplaintCode.BREATHING]: (intake) => {
    ...
    if (
      severity === "Severe (single words only)" ||
  ...
  [ComplaintCode.INJURY]: (intake) => {
    ...
    if (
      r["injury_type"] === "Motor vehicle accident" ||
  ...
  [ComplaintCode.ALLERGIC]: (intake) => {
    ...
    const tongueOrThroat =
      includes(r["swelling_location"], "Tongue") ||
  ...
  [ComplaintCode.SKIN_WOUND]: (intake) => {
    ...
    if (
      r["burn_appearance"] === "White or waxy" ||
...
export function evaluateSecondOrderModifiers(
