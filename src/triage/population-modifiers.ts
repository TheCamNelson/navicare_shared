...
function clampCTAS(value: number): CTASLevel {
  const v = Math.max(1, Math.min(5, value));
  return v as CTASLevel;
}

export function applyPopulationModifiers(
  currentCTAS: CTASLevel,
  intake: IntakeData,
  encounter: Encounter,
  patient: Patient,
): { ctas: CTASLevel; modifiers: PopulationModifier[] } {
  let modified: number = currentCTAS;
  const applied: PopulationModifier[] = [];
  ...
  if (
    patient.age_group === AgeGroup.SENIOR &&
  ...
  const matchCount = intake.comorbidities.filter((c) =>
    HIGH_RISK_COMORBIDITIES.includes(c),
  ).length;
  if (matchCount >= 2) {
    const before = modified;
  ...
  if (intake.comorbidities.includes("Pregnancy")) {
    const before = modified;
    if (
      encounter.complaint_code === ComplaintCode.ABDOMINAL ||
