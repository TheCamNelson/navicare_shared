...
function profileToPatient(row, email) {
...
function rowToEncounter(row) {
    return {
        ...
        state: row.state,
        complaint_code: (row.complaint_code ?? undefined),
...
function rowToIntake(row) {
    ...
    return {
        ...
        complaint_responses: (row.complaint_responses ?? {}),
    };
}
function rowToTriage(row) {
    return {
        ...
        global_red_flags_triggered: (row.global_red_flags ?? []),
        complaint_red_flags: (row.complaint_red_flags ?? []),
        first_order_modifiers: (row.first_order_modifiers ?? []),
        second_order_modifiers: (row.second_order_modifiers ?? []),
        population_modifiers_applied: (row.population_modifiers ?? []),
        complaint_derived_ctas: row.complaint_derived_ctas,
        final_ctas_level: row.final_ctas_level,
...
function rowToQueue(row) {
    return {
        ...
        clinician_type_required: (row.clinician_type_required ?? undefined),
...
function senderToDbRole(sender) {
    switch (sender) {
        case "PATIENT":
            return "patient";
        case "CLINICIAN":
...
function rowToMessage(row) {
    ...
    const meta = (row.tool_calls ?? null);
    return {
        id: row.id,
        encounter_id: row.encounter_id,
...
function rowToClinician(row, name) {
    return {
        id: row.id,
        name: name ?? "Clinician",
        license_number: row.license_number,
        clinician_type: row.clinician_type,
        is_available: row.is_available,
    };
}
function rowToReview(row, clinicianName, clinicianLicense) {
    return {
        id: row.id,
        encounter_id: row.encounter_id,
        ...
        original_ctas: row.original_ctas,
        final_ctas: row.final_ctas,
        decision: row.decision,
        override_reason: row.override_reason ?? undefined,
        notes: row.notes ?? "",
        final_pathway: row.final_pathway,
...
export class SupabaseStore {
    ...
    async createPatient(_input) {
        throw new Error("createPatient is handled by Supabase Auth signup; the on_auth_user_created trigger creates the profile row.");
    }
    async getPatient(id) {
    ...
    async findPatientByEmail(_email) {
        throw new Error("findPatientByEmail is no longer supported — use Supabase Auth (signInWithPassword) and read profiles by auth.uid().");
    }
    /* ───── Encounter ───── */
    ...
    async updateEncounter(id, patch) {
        ...
        if (patch.complaint_code !== undefined)
            update.complaint_code = patch.complaint_code ?? null;
        if (patch.is_pediatric_flag !== undefined)
            update.is_pediatric_flag = patch.is_pediatric_flag;
        if (patch.is_repeat_visit !== undefined)
            update.is_repeat_visit = patch.is_repeat_visit;
    ...
    async saveIntake(encounterId, intake) {
        ...
        const insert = {
            ...
            baseline: baseline,
            complaint_responses: (intake.complaint_responses ?? {}),
        };
    ...
    async saveTriageResult(encounterId, result) {
        ...
        const insert = {
            ...
            global_red_flags: (result.global_red_flags_triggered ?? []),
            complaint_red_flags: (result.complaint_red_flags ?? []),
            first_order_modifiers: (result.first_order_modifiers ?? []),
            second_order_modifiers: (result.second_order_modifiers ?? []),
            population_modifiers: (result.population_modifiers_applied ?? []),
    ...
    async listQueue(filter) {
        ...
        if (filter?.clinician_id)
            q = q.eq("assigned_clinician_id", filter.clinician_id);
        if (filter?.ctas_level)
            q = q.eq("ctas_level", filter.ctas_level);
    ...
    async appendMessage(encounterId, message) {
        const insert = {
            ...
            tool_calls: message.sender_id ? { sender_id: message.sender_id } : null,
        };
        ...
        const client = message.sender === "BOT" || message.sender === "SYSTEM"
            ? this.adminClient
            : this.userClient;
