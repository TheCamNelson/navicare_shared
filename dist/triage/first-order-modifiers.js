...
function assessBleedingCTAS(intake) {
    const r = intake.complaint_responses;
    const bleeding = r["active_bleeding"];
    if (bleeding === "Severe (won't stop)")
        return CTASLevel.LEVEL_1;
    if (bleeding === "Moderate")
        return CTASLevel.LEVEL_3;
    if (bleeding === "Minor")
        return CTASLevel.LEVEL_4;
    ...
    if (r["bleeding_severity"] === "Heavy (soaking pad per hour)")
        return CTASLevel.LEVEL_2;
    if (r["bleeding_severity"] === "Moderate")
        return CTASLevel.LEVEL_3;
    if (r["bleeding_severity"] === "Spotting")
        return CTASLevel.LEVEL_4;
    return CTASLevel.LEVEL_4;
}
function hasBleedingIndicators(intake) {
    const r = intake.complaint_responses;
    return (typeof r["active_bleeding"] !== "undefined" &&
        r["active_bleeding"] !== "No" ||
        r["bleeding_controlled"] === "No" ||
        (typeof r["bleeding_severity"] === "string" &&
            r["bleeding_severity"] !== "None"));
}
function assessMOI(intake) {
    const r = intake.complaint_responses;
...
export function evaluateFirstOrderModifiers(complaint, intake) {
    const modifiers = [];
    // Pain severity
...
//# sourceMappingURL=first-order-modifiers.js.map