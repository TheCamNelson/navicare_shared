...
export function runTriage(input) {
    ...
    const firstOrder = evaluateFirstOrderModifiers(encounter.complaint_code, intake);
    // Step 3: Second-order modifiers
    const secondOrder = evaluateSecondOrderModifiers(encounter.complaint_code, intake);
    // Step 4: Highest-acuity-wins
    const allModifiers = [...firstOrder, ...secondOrder];
    let complaintCTAS;
    ...
    const populationResult = applyPopulationModifiers(complaintCTAS, intake, encounter, patient);
    // Step 6: Auto-escalation safety rules
    const finalCTAS = applyAutoEscalationRules(populationResult.ctas, encounter, intake);
